...
#               
#               At module address 2, testscript is only allowed to run on:
#               Channel 5, A + B
#               Channel 6, A + B
#               Channel 7, A + B
#               Channel 8, A + B
...
# 2017-01-18 : Added cycle counter en voltage / temperature readout
#-----------------------------------------------------------------------------#
#                                                                             #
...
DELAY_AFTER_SWITCH_OFF = 0.5
DELAY_AFTER_SWITCH_ON  = 0.5
...
channels_to_test = {
    ...
    3 : [1, 2, 3, 4, 5, 6, 7, 8],
    4 : [1, 2, 3, 4, 5, 6, 7, 8],
    5 : [1, 2, 3, 4, 5],            # special case, channels 6..8 not connected on this module
    6 : [1, 2, 3, 4, 5, 6, 7, 8]}
...
def Initialize (interface):
    ...
    # Note that as a side effect of 'init' to 'reg-command', the output control register gets
    # set, which immediately enables the physical outputs
...
def RunTest (interface):
    ...
    for module in range (FIRST_MODULE, LAST_MODULE + 1):
        ...
        temperature = interface.GetTemperature (module)
        print '  Temperature: ', temperature, u'\u00b0''C'
        voltage = interface.GetVoltage (module)
...
if (__name__ == '__main__'):
    ...
    if (len (ports) < 1):
        ...
        # Initialize and switch on all outputs
        Initialize (interf)
        
        # create an empty line in the output stream
        print ''
        
        # for ever run the test
        # TODO: see if we can catch the KeyboardInterrupt exception
        #       On the other hand, the script can also be started from the windows exploder, after which
        #       stopping it by closing the window would never trigger the keyboard interrupt.
        #       So now we rely in the python interpreter to do a decent shutdown and release the comport...
        num_cycles = 1
        while 1:
            print 'Starting cycle:', num_cycles
            RunTest (interf)
            num_cycles += 1
