...
class clsStatusBar (Tkinter.Frame):
    ...
    def __init__ (self, master, main_frame, nr_panes = 1, pane_sizes = []):
        ...
        sum = 0.0
        for cnt in xrange (len (self._pane_sizes)):
        ...
        xpos = 0.0
        ypos = 0.0
        for cnt in xrange (self._nr_panes):
...
class clsNotebook (object):
    ...
    def _id_to_index (self, id):
        ...
        index = -1
        for cnt in xrange (len (self._frame_id_list)):
    ...
    def _frame_resize (self, event = None):
        ...
        button_width      = 0
        for cnt in xrange (len (self._button_list)):
    ...
    def _go_left (self, event = None):
        ...
        if (self._is_most_left > 0):
            ...
            for cnt in xrange (self._is_most_left, len (self._button_list)):
                self._button_list [cnt].pack (fill = Tkinter.BOTH, side = self._side)
    ...
    def _close_handler (self, id):
        ...
        for cnt in xrange (len (self._on_close_list)):
    ...
    def _display (self, fr):
        ...
        # call on_change handlers if there are any
        for cnt in xrange (len (self._on_change_list)):
...
class clsComboBox (Tkinter.Frame):
    def __init__ (self, parent, DropListSize = 5):
        ...
        # Drop Labels
        self.Labels = [self.CreateDropLabel (i) for i in xrange (DropListSize)]
    ...
    def HandleChange (self, *args):
        for cnt in xrange (len (self.ChangeClients)):
...
#-----------------------------------------------------------------------------#
class clsAskStringFromList (tkSimpleDialog.Dialog):
    DLG_WIDTH  = 225
    DLG_HEIGHT = 200
...
#-----------------------------------------------------------------------------#
class clsSettings (tkSimpleDialog.Dialog):
    BASE_X_POS       = 5
    X_INDENT         = 25
    X_DATA1_INDENT   = 325
    X_DATA2_INDENT   = 450
    ...
    #-----
    # main body of the dialog; this function is called by calling tkSimpleDialog.Dialog.__init__ at the
...
#-----------------------------------------------------------------------------#
class clsUploadDialog (tkSimpleDialog.Dialog):
